...
const CHALLENGES = [
  { icon: Bot, title: "Chatbots that can't take action", desc: "Most conversational tools answer questions but cannot actually execute the task behind them." },
  { icon: ShieldAlert, title: "No oversight on autonomous decisions", desc: "Agents making consequential decisions without approval checkpoints create real operational risk." },
  { icon: GitBranch, title: "Agents that don't know when to stop", desc: "Without clear boundaries, autonomous systems can take incorrect actions with no human in the loop." },
  { icon: Database, title: "No access to real business systems", desc: "Agents disconnected from your actual data and tools can only simulate usefulness, not deliver it." },
  { icon: Repeat, title: "Fragile multi-step reasoning", desc: "Simple agent chains break down on tasks that require coordinating several steps reliably." },
  { icon: Eye, title: "No visibility into agent behavior", desc: "Leadership cannot see what an autonomous agent did, why, or whether it should be trusted again." },
];
...
const DELIVERABLES = [
  { icon: Bot, title: "Task-Specific Agents", desc: "Agents scoped to a defined business task, not an open-ended general assistant." },
  { icon: Workflow, title: "Multi-Step Orchestration", desc: "Agents that plan and execute sequences of actions across multiple systems." },
  { icon: Database, title: "System-Connected Agents", desc: "Direct integration with your business tools, data, and internal APIs." },
  { icon: ShieldCheck, title: "Approval & Guardrail Framework", desc: "Defined checkpoints and limits controlling what an agent can act on autonomously." },
  { icon: Eye, title: "Agent Activity Monitoring", desc: "Full visibility into every action an agent takes, with complete audit logs." },
...
const FAQS = [
  ...
  { q: "What stops an agent from taking an incorrect or risky action?", a: "Every agent is built with explicit guardrails: scoped permissions, approval checkpoints for consequential actions, and hard limits on what it can do without human sign-off." },
  { q: "Can agents work across multiple internal systems?", a: "Yes. Agents are connected directly to your business tools and APIs, so they can coordinate actions across ERP, CRM, and internal systems as part of a single workflow." },
  { q: "How do we monitor what an agent has done?", a: "Every action an agent takes is logged with full context, giving your team a complete, reviewable history of agent behavior for audit and troubleshooting." },
  { q: "Do agents require constant human supervision?", a: "No, but they are not left fully unsupervised either. Routine actions run autonomously within scope, while consequential decisions are routed to a human for approval before execution." },
];
    ...
    <div className="min-h-screen bg-white text-[#0B1120]">
      ...
      <section className="relative h-screen min-h-[720px] w-full overflow-hidden flex items-end">
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto w-full px-6 md:px-10 pb-24 pt-40">
          ...
          >
            Autonomous agents that get real work done.
          </motion.h1>
          <motion.p
            variants={fadeUp}
            initial="hidden"
          ...
          >
            ...
            coordinate work across your tools and data, with guardrails
            that keep people in control of every consequential decision.
          </motion.p>
          <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={0.32}>
            ...
            >
              Schedule an Agent Scoping Session
      ...
      <section className="py-28 px-6 md:px-10 bg-white">
        <div className="max-w-[1400px] mx-auto grid md:grid-cols-2 gap-16 items-center">
          ...
          >
            <Eyebrow>Why AI Agents</Eyebrow>
            <h2 className="text-3xl md:text-5xl font leading-[1.1] mb-8 tracking-tight text-[#0B1120]">
              An assistant that only talks isn't the same as one that acts.
            </h2>
            <div className="text-gray-600 text-lg leading-relaxed space-y-5">
              <p>
                Most conversational AI tools stop at generating a response.
                They can summarize, draft, and explain, but they cannot
                query a system, update a record, or complete a multi-step
                task without a person doing the actual work behind the
                conversation.
              </p>
              <p>
                An agent is built to close that gap. It plans a sequence of
                steps, calls the systems it needs, and executes the task —
                within boundaries defined in advance, with approval
                checkpoints on anything consequential, so autonomy never
      ...
      <section className="relative py-40 px-6 md:px-10 overflow-hidden">
        ...
        >
          <h2 className="text-4xl md:text-6xl font leading-[1.05] text-white mb-8 tracking-tight">
            Give your team an agent that actually finishes the task.
          </h2>
          <p className="text-white/70 text-lg max-w-2xl mx-auto mb-12 leading-relaxed">
            Our agent engineers help you scope, build, and govern autonomous
            systems that execute real work within boundaries you control.
          </p>
          <Link
            to="/contact"
